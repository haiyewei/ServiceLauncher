...
import 'package:path_provider/path_provider.dart'; // Added for log file path
...
class Service {
  ...
  factory Service.fromJson(Map<String, dynamic> json) {
    ...
    return Service(
      ...
      preParameters:
          json['preParameters'] ??
          '', // Handle potential null or missing field for new field
      postParameters:
          json['postParameters'] ??
  ...
  factory PersistedServiceRunState.fromJson(Map<String, dynamic> json) {
    return PersistedServiceRunState(
      desiredState:
          ...
          json['desired_state'] as String? ??
          'stopped',
      lastKnownPid:
          json['lastKnownPid'] as int? ?? json['last_known_pid'] as int?,
      intentionallyStopped:
          json['intentionallyStopped'] as bool? ??
          json['intentionally_stopped'] as bool? ??
          false,
      autoStartPreference:
          json['autoStartPreference'] as bool? ??
          json['auto_start_preference'] as bool? ??
...
class TerminalController extends ChangeNotifier {
  ...
  final Map<String, PersistedServiceRunState> _serviceRunStates =
      {}; // Stores serviceId to its run state
  final Map<String, Process> _activeProcesses =
      {}; // Stores serviceId to actual Process object

  // Map to store terminal output for each service, keyed by service ID
  final Map<String, ValueNotifier<String>> _terminalOutput = {};
  // Map to store log file watchers
  final Map<String, StreamSubscription<FileSystemEvent>> _logFileSubscriptions =
      {};
  ...
  final int maxLogLines = 1000; // Maximum lines to keep in terminal output
  ...
  Set<String> get runningProcessIds =>
      _serviceRunStates.entries
          .where(
            (entry) =>
                entry.value.desiredState == 'running' &&
                entry.value.lastKnownPid != null,
          )
          .map((entry) => entry.key)
          .toSet();
  ...
  Future<void> _setupLoggingForRunningService(
    String serviceId,
    Service serviceDefinition,
    int pid,
    String baseMessage,
  ) async {
    if (!_terminalOutput.containsKey(serviceId) ||
        _terminalOutput[serviceId] == null) {
      _terminalOutput[serviceId] = ValueNotifier<String>('');
    }
    // _terminalOutput[serviceId]!.value = baseMessage; // Start with the base message
    _appendToOutputAndLog(serviceId, serviceDefinition.name, baseMessage);

    if (serviceDefinition.logFilePath != null &&
        serviceDefinition.logFilePath!.isNotEmpty) {
      ...
      try {
        if (await logFile.exists()) {
          ...
          _appendToOutputAndLog(
            serviceId,
            serviceDefinition.name,
            '[Tailing log file: ${serviceDefinition.logFilePath} for service \'${serviceDefinition.name}\' (PID: $pid)]\n',
          );
          ...
          _watchLogFile(
            ...
            serviceDefinition.name,
            logFile,
          ); // This method already appends a "watching" message
          debugPrint(
            'Service ${serviceDefinition.name} (ID: $serviceId) PID $pid. Tailing log: ${serviceDefinition.logFilePath}',
          );
        } else {
          ...
          _appendToOutputAndLog(
            serviceId,
            serviceDefinition.name,
            '[Log file specified (${serviceDefinition.logFilePath}) but not found. Live output may be unavailable.]\n',
          );
          debugPrint(
            'Service ${serviceDefinition.name} (ID: $serviceId) PID $pid. Log file not found: ${serviceDefinition.logFilePath}',
          );
        }
      } catch (e) {
        ...
        _appendToOutputAndLog(
          serviceId,
          serviceDefinition.name,
          '[Error accessing log file (${serviceDefinition.logFilePath}): $e. Live output may be unavailable.]\n',
        );
        debugPrint(
          'Error accessing log file for ${serviceDefinition.name} (PID $pid): $e',
        );
      }
    } else {
      ...
      _appendToOutputAndLog(
        serviceId,
        serviceDefinition.name,
        '[Service \'${serviceDefinition.name}\' (PID: $pid). Live output capture requires a configured log file, which is not specified or accessible.]\n',
      );
      debugPrint(
        'Service ${serviceDefinition.name} (ID: $serviceId) PID $pid. No log file configured.',
      );
    }
  }
  ...
  Future<void> _loadRunningState() async {
    ...
    try {
      ...
      if (await file.exists()) {
        ...
        if (contents.isEmpty) {
          ...
          for (var service in _services) {
            // This loop might be redundant if _loadServices hasn't run yet, or if _services is empty.
            // It's safer to do this after _loadServices.
            if (!_serviceRunStates.containsKey(service.id)) {
              _serviceRunStates[service.id] =
                  PersistedServiceRunState.defaultState();
            }
          }
          return;
        }
        ...
        for (final entry in loadedJson.entries) {
          ...
          if (stateJson is Map<String, dynamic>) {
            ...
            final serviceDefinition = _services.firstWhereOrNull(
              (s) => s.id == serviceId,
            );
            if (serviceDefinition != null) {
              // Only process states for known services
              _serviceRunStates[serviceId] = PersistedServiceRunState.fromJson(
                stateJson,
              );

              // Logging setup for recovered running services
              final currentLoadedState = _serviceRunStates[serviceId];
              if (currentLoadedState?.desiredState == 'running' &&
                  currentLoadedState?.lastKnownPid != null) {
                _startTimes[serviceId] =
                    DateTime.now(); // Placeholder for recovered start time
                String recoveryMessage =
                    'Service \'${serviceDefinition.name}\' (PID: ${currentLoadedState?.lastKnownPid}) state loaded from ${p.basename(filePath)}.\nAttempting to re-attach or verify...\n';
                // Call _setupLoggingForRunningService here
                if (currentLoadedState?.lastKnownPid != null) {
                  // Check lastKnownPid again for safety, though outer condition implies it
                  await _setupLoggingForRunningService(
                    serviceId,
                    serviceDefinition,
                    currentLoadedState!.lastKnownPid!,
                    recoveryMessage,
                  );
                } else {
                  ...
                  _appendToOutputAndLog(
                    serviceId,
                    serviceDefinition.name,
                    '$recoveryMessage[Error: Recovered with running state but PID is null. Cannot tail logs.]\n',
                  );
                }
                debugPrint(
                  'Service ${serviceDefinition.name} (ID: $serviceId) state loaded from ${p.basename(filePath)}: desiredState=${currentLoadedState?.desiredState}, pid=${currentLoadedState?.lastKnownPid}',
                );
              }
            } else {
              debugPrint(
                'Service ID "$serviceId" from ${p.basename(filePath)} not found in current service configurations. Its persisted state will be ignored and removed on next save if still not configured.',
              );
            }
          } else {
            debugPrint(
              'Invalid state format for service ID "$serviceId" in ${p.basename(filePath)}. Expected a Map.',
            );
          }
        }
        ...
        debugPrint(
          'Running state file $filePath not found. Initializing default states for all configured services.',
        );
      ...
      debugPrint(
        'Error loading running state from $filePath: $e. Initializing with default states.',
      );
      ...
      try {
        ...
        debugPrint(
          'Failed to delete corrupt running state file $filePath: $deleteError',
        );
      }
    }
    ...
    for (var service in _services) {
      if (!_serviceRunStates.containsKey(service.id)) {
        ...
        debugPrint(
          'Created default run state for service not found in running.json: ${service.name} (ID: ${service.id})',
        );
      }
    }
  }
  ...
  */
  ...
  Future<void> _writeToServiceLogFile(
    String serviceId,
    String serviceName,
    String content,
  ) async {
    if (content.trim().isEmpty) return; // Don't write empty lines
    ...
    try {
      ...
      // Sanitize serviceName for use in filename if necessary, or use serviceId if name is problematic
      final sName = serviceName
          .replaceAll(RegExp(r'[^\w\s-]'), '_')
          .replaceAll(' ', '_');
      final logFileName = '${sName}_$dateStr.log';
      final logFile = File(p.join(logsDir.path, logFileName));

      await logFile.writeAsString(content, mode: FileMode.append, flush: true);
    } catch (e) {
      debugPrint(
        'Error writing to service log file for $serviceName (ID: $serviceId): $e',
      );
      // Optionally, write this error to the terminal output itself if it's critical
      // _appendToOutputAndLog(serviceId, serviceName, '[ERROR: Could not write to service log file: $e]\n');
    }
  }

  void _appendToOutputAndLog(
    String serviceId,
    String serviceName,
    String newData,
  ) {
    ...
    var currentOutput = _terminalOutput[serviceId]!.value;
    currentOutput += newData;

    // Log truncation
    var lines = currentOutput.split('\n');
    if (lines.length > maxLogLines) {
      // If the last "line" is not empty (meaning currentOutput didn't end with \n),
      // it's part of the last line and should be preserved.
      // Otherwise, if it ends with \n, the split will produce an empty string at the end.
      int linesToKeep = maxLogLines;

      // If the current output doesn't end with a newline, the last element of `lines` is an incomplete line.
      // We want to keep `maxLogLines` complete lines plus this potentially incomplete one.
      // So, we take `lines.length - linesToKeep` as the starting index.
      // Example: maxLogLines = 3. lines = ["a", "b", "c", "d", "e"] (length 5)
      // We want to keep "c", "d", "e". Start index = 5 - 3 = 2. lines.sublist(2)
      // Example: maxLogLines = 3. lines = ["a", "b", "c", "d", "e", ""] (length 6, ends with \n)
      // We want to keep "d", "e", "". Start index = 6 - 3 = 3. lines.sublist(3)

      // A simpler way: take the last `maxLogLines` elements.
      lines = lines.sublist(
        lines.length > linesToKeep ? lines.length - linesToKeep : 0,
      );
    }
    _terminalOutput[serviceId]!.value = lines.join('\n');

    // Write to service log file
    _writeToServiceLogFile(serviceId, serviceName, newData);
  }

  void _watchLogFile(String serviceId, String serviceName, File logFile) async {
    // Cancel any existing subscription for this serviceId
    await _logFileSubscriptions[serviceId]?.cancel();
    _logFileSubscriptions.remove(serviceId);
    ...
    try {
      ...
      final Stream<FileSystemEvent> watcher = logFile.watch(
        events: FileSystemEvent.modify | FileSystemEvent.create,
      );

      await Future.delayed(const Duration(milliseconds: 250));

      _logFileSubscriptions[serviceId] = watcher.listen(
        (event) async {
          try {
            if (event.type == FileSystemEvent.modify ||
              ...
              if (currentSize > lastSize) {
                final randomAccessFile = await logFile.open(
                  mode: FileMode.read,
                );
                ...
                if (newContent.isNotEmpty) {
                  ...
                  _appendToOutputAndLog(serviceId, serviceName, newContent);
                }
                ...
                await randomAccessFile.setPosition(lastSize);
                final newBytes = await randomAccessFile.read(currentSize);
                await randomAccessFile.close();
                final newContent = utf8.decode(newBytes);
                if (newContent.isNotEmpty) {
                  // _terminalOutput[serviceId]?.value += newContent;
            ...
            await _logFileSubscriptions[serviceId]
                ?.cancel(); // Attempt to cancel on error
            _logFileSubscriptions.remove(serviceId);
          }
        },
        onError: (error) {
          ...
          _logFileSubscriptions.remove(
            serviceId,
          ); // Already cancelled by cancelOnError: true
        },
        onDone: () {
          logFile.exists().then((exists) {
            ...
            debugPrint(
              "Log file watcher stream done for $serviceId (${logFile.path}).",
            );
          });
          _logFileSubscriptions.remove(serviceId);
        },
        ...
        _services =
            jsonList.map((json) {
              return Service.fromJson(
                json,
              ); // fromJson now handles ID generation
            }).toList();
      } else {
  ...
  Future<void> updateService(Service updatedService) async {
    ...
    if (index != -1) {
      ...
      debugPrint(
        "Error: Service with ID ${updatedService.id} not found for update.",
      );
  ...
  Future<void> deleteService(String serviceId) async {
    ...
    final serviceToDelete = _services.firstWhere(
      (s) => s.id == serviceId,
      orElse: () => Service(id: '', name: '', type: '', path: ''),
    ); // Provide a dummy service if not found

    _services.removeWhere((s) => s.id == serviceId);
    if (_services.length < initialLength) {
  ...
  Future<bool> isPidActuallyRunning(int? pid) async {
    if (pid == null || pid == 0) {
      return false; // PID 0 is usually the scheduler, not a user process.
    }
  ...
  Future<bool> startService(Service service, {bool forceStart = false}) async {
    ...
    if (!forceStart &&
        ...
        currentRunState.desiredState == 'running' &&
        currentRunState.lastKnownPid != null) {
      bool actuallyRunning = await isPidActuallyRunning(
        currentRunState.lastKnownPid!,
      );
      if (actuallyRunning) {
        debugPrint(
          'Service ${service.name} is already running with PID ${currentRunState.lastKnownPid}. Start request ignored (forceStart is false).',
        );
        if (_activeProcesses.containsKey(service.id)) {
          // Already active, logging should be fine
        } else {
          String recoveryMessage =
              'Service \'${service.name}\' (PID: ${currentRunState.lastKnownPid}) was found running (verified). Re-attaching/logging.\n';
          await _setupLoggingForRunningService(
            service.id,
            service,
            currentRunState.lastKnownPid!,
        ...
        debugPrint(
          'Service ${service.name} was marked running with PID ${currentRunState.lastKnownPid}, but PID is dead. Proceeding to (re)start.',
        );
        ...
        currentRunState.desiredState == 'running') {
      debugPrint(
        'Force starting service ${service.name}. Previous PID was ${currentRunState.lastKnownPid}.',
      );
      ...
      if (currentRunState.lastKnownPid != null) {
        ...
        bool oldPidStillRunning = await isPidActuallyRunning(
          currentRunState.lastKnownPid,
        );
        if (oldPidStillRunning) {
          debugPrint(
            'Force start: Attempting to kill old process PID ${currentRunState.lastKnownPid} for service ${service.name}',
          );
          ...
          try {
            ...
            debugPrint(
              'Force start: Old process PID ${currentRunState.lastKnownPid} for ${service.name} likely killed.',
            );
          } catch (e) {
            debugPrint(
              'Force start: Error killing old process PID ${currentRunState.lastKnownPid} for ${service.name}: $e',
            );
          }
        }
      }
    ...
    try {
      ...
      if (service.preCommand.isNotEmpty) {
        ...
        if (service.programName.isNotEmpty) {
          ...
          if (service.programName.contains('/') ||
              service.programName.contains('\\') ||
              FileSystemEntity.isFileSync(
                p.join(service.path, service.programName),
              )) {
            arguments.add(p.join(service.path, service.programName));
          } else {
            arguments.add(
              service.programName,
            ); // Assume it's an argument like 'start' for 'npm'
          }
        }
        ...
        if (service.programName.isEmpty) {
          ...
          NotifyController().showNotify(
            NotifyData(
              message: '启动服务 ${service.name} 失败: 服务程序名称为空。',
              type: NotifyType.app,
              time: DateTime.now(),
              icon: Icons.error_outline,
            ),
      ...
      _serviceRunStates[service.id] = PersistedServiceRunState(
        ...
        autoStartPreference:
            _serviceRunStates[service.id]?.autoStartPreference ??
            true, // Retain existing or default
      );
      ...
      process.stdout.transform(utf8.decoder).listen((data) {
        ...
        _appendToOutputAndLog(service.id, service.name, data);
      });
      ...
      process.exitCode.then((exitCode) async {
        ...
        final exitedPid =
            _serviceRunStates[service.id]
                ?.lastKnownPid; // Get PID before clearing state
        debugPrint(
          'Service ${service.name} (PID: $exitedPid) exited with code $exitCode',
        );
      ...
      debugPrint(
        'Service ${service.name} started successfully with PID ${process.pid}.',
      );
      ...
      if (_serviceRunStates.containsKey(service.id)) {
        ...
        _serviceRunStates[service.id] =
            PersistedServiceRunState.defaultState(
                autoStart:
                    _serviceRunStates[service.id]?.autoStartPreference ?? true,
              )
              ..desiredState = 'stopped'
              ..lastKnownPid = null;
      }
  ...
  Future<bool> startServiceById(
    ...
    if (service != null) {
      ...
      if (!forceStart &&
        ...
        if (actuallyRunning) {
          debugPrint(
            'Service ${service.name} (ID: $serviceId) is already running with PID ${currentRunState.lastKnownPid}. Skipping start attempt via startServiceById (forceStart is false).',
          );
          ...
          debugPrint(
            'Service ${service.name} (ID: $serviceId) was marked running with PID ${currentRunState.lastKnownPid}, but PID is dead. Proceeding to start via startServiceById.',
          );
        }
      }
    ...
    NotifyController().showNotify(
      NotifyData(
        message: '启动服务失败: 未找到ID为 $serviceId 的服务。',
        type: NotifyType.app,
        time: DateTime.now(),
        icon: Icons.error_outline,
      ),
  ...
  Future<bool> stopService(String serviceId) async {
    ...
    if (currentRunState == null || currentRunState.desiredState == 'stopped') {
      ...
      if (currentRunState == null && getServiceById(serviceId) != null) {
        _serviceRunStates[serviceId] = PersistedServiceRunState.defaultState(
          autoStart: false,
    ...
    final pid =
        currentRunState
            .lastKnownPid; // PID can be null if desiredState was 'running' but pid was lost
    ...
    if (pid == null || !actuallyRunning) {
      ...
      _serviceRunStates[serviceId] = PersistedServiceRunState(
        ...
        autoStartPreference:
            currentRunState.autoStartPreference, // Preserve preference
      );
      _activeProcesses.remove(serviceId);
      _startTimes.remove(serviceId);
      // Cancel and remove log file subscription if it exists
      await _logFileSubscriptions[serviceId]?.cancel();
      _logFileSubscriptions.remove(serviceId);
      await _saveRunningState();
    ...
    try {
      if (Platform.isWindows) {
        ...
        taskkillResult = await Process.run('taskkill', [
          '/PID',
          '$pid',
          '/T',
          '/F',
        ...
        if (taskkillResult.exitCode == 0) {
          ...
          killAttemptDetails +=
              "失败 (退出码: ${taskkillResult.exitCode}, stderr: ${taskkillResult.stderr.toString().trim()}).\n";
          if (process != null) {
            killAttemptDetails += "尝试使用 process.kill() (PID: $pid) 作为备用方案... ";
            debugPrint(
              'taskkill failed for PID $pid. Attempting process.kill() as fallback.',
            );
            try {
            ...
            debugPrint(
              'taskkill failed for PID $pid, and no active Process object for fallback.',
            );
          }
        }
      } else {
        // Non-Windows
        killAttemptDetails +=
            "尝试使用 process.kill() (PID: $pid) 在非 Windows 平台... ";
        if (process != null) {
          killSuccess = process.kill();
          killAttemptDetails += killSuccess ? "成功。\n" : "失败。\n";
        } else {
          ...
          debugPrint(
            'Service $serviceName (PID: $pid) is a recovered service on non-Windows. Cannot send SIGTERM without a Process object.',
          );
          killSuccess = false;
        }
      }
      ...
      if (killSuccess || wasRunningAndNowStopped) {
        // If kill worked or process died anyway
        _serviceRunStates[serviceId] = PersistedServiceRunState(
          desiredState: 'stopped',
          lastKnownPid: null,
          intentionallyStopped: true, // User initiated stop
        ...
        if (_serviceRunStates.containsKey(serviceId)) {
          // Check if key exists before modifying
          _serviceRunStates[serviceId]!.intentionallyStopped = true;
          _serviceRunStates[serviceId]!.desiredState =
              'stopped'; // User wants it stopped
      ...
      // _terminalOutput[serviceId]?.value += '\n$killAttemptDetails';
      _appendToOutputAndLog(serviceId, serviceName, '\n$killAttemptDetails');
      ...
      if (killSuccess) {
        ...
        _appendToOutputAndLog(
          serviceId,
          serviceName,
          '停止服务 $serviceName (PID: $pid) 失败。原因: $failureReason\n',
        );
      }
      notifyListeners();
    }
  ...
  void dispose() {
    ...
    List<String> idsKilledOnDispose = List.from(
      _serviceRunStates.keys,
    ); // Iterate over a copy
    for (String id in idsKilledOnDispose) {
      if (_serviceRunStates.containsKey(id) &&
          _serviceRunStates[id]!.desiredState == 'running') {
    ...
    // Dispose all log file subscriptions
    _logFileSubscriptions.forEach((id, subscription) {
      subscription.cancel();
    });
    _logFileSubscriptions.clear();
